...
export default class MyToster {
  ...
  removeItem(item){
    gsap.fromTo(item, 0.25, {xPercent: 0, ease: this.ease_0}, {xPercent: 100, onComplete: ()=>{
      item.remove();
    }});
  }
  addToast(settingObj){
    ...
    /*  */
    gsap.fromTo(item, 0.75, {xPercent: 100, skewX: -10}, {xPercent: 0, skewX: 0, ease: this.ease_0});
	...
	listeners(input) {
    const self = this;
    this.$wrap.addEventListener('click', function ({target}) {
